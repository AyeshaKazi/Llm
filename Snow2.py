...
for i in range(len(queries) - 1):
    # Skip connection from Query 4 to Query 5
    if i == 3:
        tree.edge(f"Q{i+1}", f"Q{6}")  # Connect Query 4 to Query 6
    elif i == 5:
        continue  # Skip Query 5 in the flow
    else:
        tree.edge(f"Q{i+1}", f"Q{i+2}")  # Default sequential connection
...
tree.render('tree_with_merged_queries_updated', view=True)
